            ...
            if user is not None:
                ...
                if profile_obj is None:
                    profile_obj = Profile.objects.create(
                        user=user, auth_token=str(uuid.uuid4())
                    )
                    profile_obj.save()
                    resend_link = reverse("resend_email_verification", args=[user.id])
                    data = {
                        "response": "Verify your email to reopen your account <a href="
                        + resend_link
                        + "> Click here</a>",
                        "result": "fail",
                    }
                ...
                if profile_obj is not None and not profile_obj.is_verified:
                    resend_link = reverse("resend_email_verification", args=[user.id])
                    data = {
                        "response": "Profile is not verified check your mail or Click resend to send the verification email. <a href="
                        + resend_link
                        + "> Resend</a>",
                        "result": "fail",
                    }
                ...
                data = [
                    {
                        "response": "Invalid password please enter correct password.",
                        "result": "fail",
                    }
                ]
                return HttpResponse(json.dumps(data), content_type="application/json")
...
def getCourses(request):
    ...
    data = [
        {"name": course.name, "title": course.title, "id": course.id}
        ...
        if type == "ca":
            ...
            for file in File:
                cloudinary.api.delete_resources(
                    file.fileupload, resource_type="raw", type="upload"
                )
            File.delete()
...
def send_mail_after_registration(email, username, token):
    ...
    html_message = render_to_string(
        html_template,
        {"token": token, "verification_link": verification_link, "username": username},
    )
...
def verify_mail_after_registration(request, auth_token):
    ...
    if profile_obj:
        ...
        time_difference = (
            current_time - profile_obj.created_at
        )  # Assuming you have a 'created_at' field in your Profile model
        ...
        if user:
            ...
            messages.success(
                request, "Verification email has been resent. Check your mail."
            )
            return redirect("login")
        else:
            return render(request, "base/Error.html")
        ...
        try:
            ...
            if language == "python":
                ...
                try:
                    ...
                    result = {"result": process.stdout, "error": process.stderr}
                    return JsonResponse(result)
    ...
    if request.method == "POST":
        ...
        if user_data.exists():
            for user in user_data:
                ...
                user_verification = User_Email_verification.objects.create(
                    user=user, email=email, otp=otp_token, auth_token=auth_token_str
                )
                ...
                messages.success(
                    request, "Enter the 6 digit OTP send on your Registered Email id"
                )
    ...
    if not "email" in request.session:
        ...
        if request.method == "POST":
            ...
            if "otp_generated_time" in request.session:
                ...
                otp_generated_time = datetime.strptime(
                    otp_generated_time_str, "%Y-%m-%d %H:%M:%S.%f%z"
                )
                current_time = timezone.now()
            ...
            user_verification = User_Email_verification.objects.filter(
                user__email=email
            ).first()
            ...
            if user_verification and otp_token == entered_otp:
                ...
                return render(
                    request, "base/password_verification.html", {"form": form}
                )
        ...
        if user_data.exists():
            for user in user_data:
                ...
                if email.send() == True:
                    messages.success(
                        request,
                        "Enter the new 6-digit OTP sent to your registered email id.",
                    )
                    return render(
                        request, "base/otp-verification.html", {"form": userForm()}
                    )
                else:
                    return render(request, "base/Error.html")
            ...
            if new_password == confirm_password:
                ...
                if reset_email:
                    ...
                    if user_queryset.exists():
                        ...
                        messages.error(
                            request, "User with the specified email not found."
                        )
                else:
                    messages.error(
                        request, "Email session no longer available. Session expired!!!"
                    )
            else:
                messages.error(
...
def uploadFileAsAdmin(request):
    if request.method == "POST":
        try:
            ...
            fileData = files(
                title=Title, courseCode=Coursecode, unit=Unit, fileupload=File
            )
            fileData.save()
            ...
            for file in files:
                ...
                if file_extension in (".jpg", ".jpeg", ".png"):
                    ...
                    with transaction.atomic():
                        existing_record = (
                            cafiles.objects.select_for_update()
                            .filter(user=user, cadate=cadate)
                            .first()
                        )
                        ...
                        if existing_record:
                            ...
                            existing_record.fileupload.save(
                                "merged_file.pdf", ContentFile(pdf_content), save=True
                            )

                            data = [
                                {"response": "CA File Updated", "result": "success"}
                            ]
                            return HttpResponse(
                                json.dumps(data), content_type="application/json"
                            )
                ...
                data = [
                    {
                        ...
                        "coursefilescount": Files.count(),
                        "cafilescount": Cafiles.count(),
...
def returnSubscribers(request):
    ...
    if request.user in admins:
        try:
            Subscribers = subscribers.objects.all()
            data = [
                {"email": subscribe.email, "id": subscribe.id}
            ...
            data = [
                {
                    ...
                    "isverified": is_Verified(user.username),
                    "lastlogin": json_serial(user.last_login),
                    "id": user.id,
                }
                for user in users
            ]
        ...
        data = [
            {
                ...
                "profilephoto": (
                    "null"
                    if get_profile_photo(request, u) == False
                    else get_profile_photo(request, u)
                ),
                "userid": u.id,
                "datejoined": json_serial(u.date_joined),
            }
            for u in user
        ]
        return HttpResponse(json.dumps(data), content_type="application/json")
            ...
            for user in users:
                ...
                if username != "":
                    if checkusername(username) or request.user.username == username:
                        ...
                        data = [
                            {"response": "Username not available", "result": "fail"}
                        ]
                        return HttpResponse(
                            json.dumps(data), content_type="application/json"
                        )
    ...
    if request.method == "POST":
        try:
            ...
            if currpwd == pwd1:
                data = [
                    {
                        "response": "Current password and new password can't be same",
                        "result": "fail",
                    }
                ]
                return HttpResponse(json.dumps(data), content_type="application/json")
            ...
            if validation == True:
                ...
                data = validation
                return HttpResponse(
                    json.dumps(validation), content_type="application/json"
                )
        except:
            data = [{"response": "Error Occured", "result": "fail"}]
            return HttpResponse(json.dumps(data), content_type="application/json")
        ...
        data = [
            {
                "title": des.title,
                "id": des.id,
                "uploaded_on": json_serial(des.Time),
                "description": des.Description,
                "votes": des.votes,
                "user": des.user.username,
                "profilephoto": (
                    "null"
                    if get_profile_photo(request, des.user) == False
                    else get_profile_photo(request, des.user)
                ),
                "tags": des.tags,
                "views": des.views,
                "url": des.Images.url,
            }
            for des in discussions
        ]
        return HttpResponse(json.dumps(data), content_type="application/json")
...
def validate(pwd1, pwd2):
    ...
    if len(pwd1) < 8 or len(pwd1) > 15:
        data = [
            {"response": "Password must have length between 8-15", "result": "fail"}
        ]
        return data
